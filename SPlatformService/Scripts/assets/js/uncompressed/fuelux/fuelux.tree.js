...
(function ($, undefined) {//ACE
	...
	Tree.prototype = {
			...
			this.options.dataSource.data($el.data(), function (items) {
				...
				$.each(items.data, function (index, value) {
					...
					if (value.type === "folder") {
						...
						&& 'IsItemSelected' in value.additionalParameters
							&& value.additionalParameters['IsItemSelected']== true) {
							setTimeout(function(){header.trigger('click')}, 0);
					}
					 //this.selectFolder($entity);
					} else if (value.type === "item") {
						$entity = self.$element.find('.tree-item:eq(0)').clone().show();
						$entity.find('.tree-item-name').html(value.name);
						$entity.data(value);
						if (value.IsItemSelected == true) {
							$entity.addClass('tree-selected');
							$entity.find('i').removeClass(self.options['unselected-icon']).addClass(self.options['selected-icon']);

						}

						//if (value.additionalParameters['item-selected'] == true)
						//{
						//    $entity.addClass('tree-selected');
						//    $entity.find('i').removeClass(self.options['unselected-icon']).addClass(self.options['selected-icon']);
						//}
		...
		if ($el[0].attributes["disabled"]!= undefined)
				if($el[0].attributes["disabled"].value == "disabled") return;
