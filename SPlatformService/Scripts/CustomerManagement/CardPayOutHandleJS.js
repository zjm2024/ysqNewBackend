...
$(document).ready(function () {
    ...
    $("button[id*='btn_submit']").click(function () {
        ...
        if (objThirdOrder.val() == "") {
            ...
            $.ajax({
                url: _RootPath + "SPWebAPI/Card/HandleCustomerPayOut?token=" + _Token,
                type: "POST",
    ...
    $("button[id*='btn_Revenue']").click(function () {
        window.open("CardRevenueDetail.aspx?CustomerId=" + CustomerId);
    });
});
...
function Init() {
    ...
    if (PayOutHistoryId > 0) {
        load_show();
        $.ajax({
            url: _RootPath + "SPWebAPI/Card/AdminGetPayoutDetails?PayoutHistoryId=" + PayOutHistoryId + "&token=" + _Token,
            type: "Get",
            data: null,
            success: function (data) {
                console.log(data);
                if (data.Flag == 1) {
                    ...
                    var isLegitimate = data.Subsidiary;
                    if (!isLegitimate) {
                        bootbox.dialog({
                            message: "提现金额与收入不符合，请先检查该用户收入明细",
                            buttons:
                            {
                                "Confirm":
                                {
...
function SetCustomerPayOutHistory(CustomerPayOutHistoryViewVO) {
    ...
    if (CustomerPayOutHistoryViewVO.Type == 0) {
        if (CustomerPayOutHistoryViewVO.AccountName != "微信零钱")
            objType.val("银行卡提现");
        else
            objType.val("微信手动付款");
    }

    else if (CustomerPayOutHistoryViewVO.Type == 1)
        objType.val("微信自动付款");
