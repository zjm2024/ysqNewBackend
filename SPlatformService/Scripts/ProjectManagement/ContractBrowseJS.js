...
var grid = new JGrid();
function onPageInit() {
    grid.jqGrid.ID = "ContractList";
    grid.jqGrid.PagerID = "ContractListDiv";
    grid.jqGrid.Source = "../Common/GetDataJsonGrid.ashx";
    grid.jqGrid.Params = "table=ContractList";
    grid.jqGrid.LengthMenu = ["20", "50", "100", "150"];
    grid.jqGrid.DefaultSortCol = "CreatedAt";
    grid.jqGrid.DefaultSort = "desc";
    grid.jqGrid.Multiselect = true;
    grid.jqGrid.AddColumn("ContractId", "操作", false, "center", 30,
	...
	grid.jqGrid.AddColumn("ProjectName", "项目名称", true, null, 50);
	grid.jqGrid.AddColumn("CompanyName", "雇主", true, null, 50);
	...
	grid.jqGrid.AddColumn("ContractFile", "合同", true, null, 30,
        function (obj, options, rowObject) {
            var oTR = "";
            if (obj != "")
	...
	grid.jqGrid.AddColumn("BusinessStatus", "雇主状态", true, null, 40,
        function (obj, options, rowObject) {
            if (obj == '0')
                return "已创建";
            else if (obj == '1')
	...
	grid.jqGrid.AddColumn("AgencyStatus", "销售状态", true, null, 40,
        function (obj, options, rowObject) {
            if (obj == '0')
                return "已创建";
            else if (obj == '1')
...
function EditContract(contractObj) {
    window.location.href = "ContractCreateEdit.aspx?ContractId=" + $(contractObj).prev().val();
    return false;
}
