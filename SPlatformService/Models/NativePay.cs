...
namespace SPlatformService.Models
{
    public class NativePay
    {
        ...
        * @return 模式二URL
        */
        public string GetPayUrl(string out_trade_no, string productId, string total_fee)
        {
            Log.Info(this.GetType().ToString(), "Native pay mode 2 url is producing...");

            WxPayData data = new WxPayData();
            data.SetValue("body", "众销乐-资源共享众包销售平台客户钱包充值");//商品描述
            data.SetValue("attach", "钱包充值");//附加数据
            data.SetValue("out_trade_no", out_trade_no);//随机字符串
            data.SetValue("total_fee", total_fee);//总金额
            data.SetValue("time_start", DateTime.Now.ToString("yyyyMMddHHmmss"));//交易起始时间
            data.SetValue("time_expire", DateTime.Now.AddMinutes(10).ToString("yyyyMMddHHmmss"));//交易结束时间
            data.SetValue("goods_tag", "钱包充值");//商品标记
