...
namespace SPlatformService
{
    public class WxPayApi
    {
        ...
        */
        public static WxPayData OrderQuery(WxPayData inputObj, int timeOut = 6,int AppType=1)
        {
            ...
            Log.Debug("AppType:", AppType.ToString());
            AppVO AppVO = AppBO.GetApp(AppType);
            Log.Debug("AppVO:", AppVO.ToString());
            string appid = AppVO.AppId;
            string mch_id = AppVO.MCHID;

            Log.Debug("appid和mch_id", appid + "///"+ mch_id+"///"+ AppType);
            inputObj.SetValue("appid", appid);//公众账号ID
            inputObj.SetValue("mch_id", mch_id);//商户号
            //inputObj.SetValue("mchid", mch_id);//商户号
            inputObj.SetValue("nonce_str", WxPayApi.GenerateNonceStr());//随机字符串
            inputObj.SetValue("sign", inputObj.MakeSign(AppType));//签名
            ...
            string response = HttpService.Post(xml, url, false, timeOut, AppType);//调用HTTP通信接口提交数据
        ...
        */
        public static WxPayData Refund(WxPayData inputObj,string appid="", int timeOut = 6, int AppType = 1)
        {
            AppVO AppVO = AppBO.GetApp(AppType);
            string mch_id = AppVO.MCHID;

            LogBO _logBO = new LogBO(typeof(WxPayApi));
            ...
            if (appid == "")
            {
                inputObj.SetValue("appid", AppVO.AppId);//公众账号ID
            }else
            {
                inputObj.SetValue("appid", appid);//公众账号ID
            }
            ...
            inputObj.SetValue("mch_id", mch_id);//商户号
            inputObj.SetValue("nonce_str", Guid.NewGuid().ToString().Replace("-", ""));//随机字符串
            inputObj.SetValue("sign", inputObj.MakeSign(AppVO.AppType));//签名
            
            string xml = inputObj.ToXml();
            var start = DateTime.Now;

            _logBO.Info("WxPayApi:Refund request : " + xml);
            string response = HttpService.Post(xml, url, true, timeOut, AppVO.AppType);//调用HTTP通信接口提交数据到API
            _logBO.Info("WxPayApi :Refund response : " + response);
            ...
            //将xml格式的结果转换为对象以返回
            WxPayData result = new WxPayData();
            result.FromXml(response, AppVO.AppType);
        ...
        * @return 成功时返回，其他抛异常
        */
        public static WxPayData UnifiedOrder(string appid,WxPayData inputObj, int timeOut = 6, int AppType = 1)
        {
            string url = "https://api.mch.weixin.qq.com/pay/unifiedorder";
            //检测必填参数
            if (!inputObj.IsSet("out_trade_no"))
            ...
            inputObj.SetValue("appid", AppVO.AppId);//公众账号ID
            inputObj.SetValue("mch_id", AppVO.MCHID);//商户号
            inputObj.SetValue("spbill_create_ip", WxPayConfig.IP);//终端ip	  	    
            inputObj.SetValue("nonce_str", GenerateNonceStr());//随机字符串

            //签名
            inputObj.SetValue("sign", inputObj.MakeSign(AppType));
            string xml = inputObj.ToXml();

            var start = DateTime.Now;

            Log.Debug("WxPayApi", "UnfiedOrder request : " + xml);
            string response = HttpService.Post(xml, url, false, timeOut, AppType);
            Log.Debug("WxPayApi", "UnfiedOrder response : " + response);
            ...
            WxPayData result = new WxPayData();
            result.FromXml(response, AppType);
        ...
        /**
            ...
            * @return 成功时返回，其他抛异常
            */
        public static WxPayData UnifiedOrderData(WxPayData inputObj, int timeOut = 6)
        {
            string url = "https://api.mch.weixin.qq.com/pay/unifiedorder";
            //检测必填参数
            if (!inputObj.IsSet("out_trade_no"))
        ...
        * @return 成功时返回接口调用结果，其他抛异常
        */
        public static WxPayData Profitsharing(WxPayData inputObj, string appid = "", int AppType = 1)
        {
            AppVO AppVO = AppBO.GetApp(AppType);
            string mch_id = AppVO.MCHID;
            ...
            {
                throw new WxPayException("分账申请接口中，缺少必填参数transaction_id！");
            }
            else if (!inputObj.IsSet("out_order_no"))
            {
                throw new WxPayException("分账申请接口中，缺少必填参数out_order_no！");
            }
            ...
            inputObj.SetValue("mch_id", mch_id);//商户号
            inputObj.SetValue("nonce_str", Guid.NewGuid().ToString().Replace("-", ""));//随机字符串
            inputObj.SetValue("sign", inputObj.MakeSignHMAC(AppVO.AppType));//签名

            string xml = inputObj.ToXml();
            var start = DateTime.Now;

            _logBO.Info("WxPayApi:Profitsharing request : " + xml);
            string response = HttpService.Post(xml, url, true, 6, AppVO.AppType);//调用HTTP通信接口提交数据到API
            _logBO.Info("WxPayApi :Profitsharing response : " + response);
        ...
        * @return 成功时返回接口调用结果，其他抛异常
        */
        public static WxPayData ProfitsharingAddReceiver(WxPayData inputObj, int AppType = 1)
        {
            AppVO AppVO = AppBO.GetApp(AppType);
            string mch_id = AppVO.MCHID;
            ...
            _logBO.Info("WxPayApi:Profitsharing request : " + xml);
            string response = HttpService.Post(xml, url, false, 6, AppVO.AppType);//调用HTTP通信接口提交数据到API
            _logBO.Info("WxPayApi :Profitsharing response : " + response);
        ...
        * 根据当前系统时间加随机序列来生成订单号
         * @return 订单号
        */
        public static string GenerateOutTradeNo(int AppType=1)
        {
            var ran = new Random();
