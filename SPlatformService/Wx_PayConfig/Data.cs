...
namespace SPlatformService
{
    ...
    public class WxPayData
    {
        ...
        * @throws WxPayException
        */
        public SortedDictionary<string, object> FromXml(string xml,int AppType=1)
        {
            ...
            try
            {
                ...
                CheckSign(AppType);//验证签名,不通过会抛异常
            }
        ...
        */
        public string MakeSign(int AppType=1)
        {
            //转url格式
            string str = ToUrl();
            //在string后加入API KEY
        ...
        * @return 签名, sign字段不参加签名
        */
        public string MakeSignHMAC(int AppType = 1)
        {
            //转url格式
            string str = ToUrl();
            //在string后加入API KEY
            ...
            //所有字符转为大写
            return CalcHMACSHA256Hash(str, AppVO.KEY);
        }
        ...
        * 正确返回true，错误抛异常
        */
        public bool CheckSign(int AppType=1)
        {
            //如果没有设置签名，则跳过检测
            if (!IsSet("sign"))
            {
            ...
            //在本地计算新的签名
            string cal_sign = MakeSign(AppType);
