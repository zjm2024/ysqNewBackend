...
namespace BroadSky.WeChatAppDecrypt
{
    ...
    public class WeChatAppDecrypt
    {
        ...
        /// <summary>
        /// 根据微信小程序平台提供的解密算法解密数据(手机)
        /// </summary>
        /// <param name="encryptedData">加密数据</param>
        /// <param name="iv">初始向量</param>
        /// <param name="sessionKey">从服务端获取的SessionKey</param>
        /// <returns></returns>
        public WechatPhoneData DecryptByPhone(string encryptedData, string iv, string sessionKey)
        {
            WechatPhoneData userInfo;
            //创建解密器生成工具实例
            AesCryptoServiceProvider aes = new AesCryptoServiceProvider();
            //设置解密器参数
            aes.Mode = CipherMode.CBC;
            ...
            string result = Encoding.UTF8.GetString(final);
            //反序列化结果，生成用户信息实例
            userInfo = JsonConvert.DeserializeObject<WechatPhoneData>(result);
            return userInfo;
        }
        /// <summary>
