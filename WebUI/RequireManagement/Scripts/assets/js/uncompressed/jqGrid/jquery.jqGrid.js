	...
	return this.each( function() {
		...
		updatepager = function(rn, dnd) {
			...
			if(pgboxes) {
				...
				var pagebutton = "";
				if (ts.p.records > 0) {
				    var template = '<input class="ui-jqgrid-pagebtn" value="{0}" style="border-radius: 8px;" type="button"/>';
				    if ((cp - 1) > 2) {
				        pagebutton += $.jgrid.format(template, 1) + "……" + $.jgrid.format(template, cp - 2) + $.jgrid.format(template, cp - 1);
				    }
				    else {
				        for (var i = 1; i < cp; i++) {
				            pagebutton += $.jgrid.format(template, i);
				        }
				    }
				    pagebutton += '<input class="ui-jqgrid-pagebtn here" value="' + cp + '" style="border-radius: 8px;" type="button"/>';

				    if ((last - cp) > 3) {
				        pagebutton += $.jgrid.format(template, cp + 1) + $.jgrid.format(template, cp + 2) + "……" + $.jgrid.format(template, last);
				    }
				    else {
				        for (var i = cp + 1; i <= last ; i++) {
				            pagebutton += $.jgrid.format(template, i);
				        }
				    }
				    pagebutton += '<input class="ui-jqgrid-pagetext" value="' + cp + '" type="text"/>';
				}
				$(ts.p.pager + "_left").html(pagebutton);

				$('input.ui-jqgrid-pagetext', pgboxes).keypress(function (e) {
				...
				$('input.ui-jqgrid-pagebtn').button().on("click", function () {
				    $(this).val(intNum($(this).val(), 1));
				    ts.p.page = ($(this).val() > 0) ? $(this).val() : ts.p.page;
				    populate();
				    return false;
		...
		setPager = function (pgid, tp){
			...
			pginp = "",
			pgl = "<table cellspacing='0' cellpadding='0' border='0' style='table-layout:auto;float:right;' class='ui-pg-table'><tbody><tr><td><span id='ui-paging-info'></span></td>",
            ...
            .append("<div id='" + pgcnt + "' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;height:100%;' role='row'><tbody><tr><td id='" + lft + "' align='left'></td><td id='" + rgt + "' align='right'></td></tr></tbody></table></div>")
			...
			if(dir==="ltr") { pgl += str; }
			pgl += "</tr></tbody></table>";
		    //if (ts.p.viewrecords === true) { $("td#" + pgid + "_" + ts.p.recordpos, "#" + pgcnt).append("<div dir='" + dir + "' style='text-align:" + ts.p.recordpos + "' class='ui-paging-info'><div style='float:right;'>" + pgl + "</div><div id='ui-paging-info' style='float:right;'></div></div>"); }
			if (ts.p.viewrecords === true) { $("td#" + pgid + "_" + ts.p.recordpos, "#" + pgcnt).append("<div dir='" + dir + "' style='text-align:" + ts.p.recordpos + "' class='ui-paging-info'>" + pgl + "</div>"); }
